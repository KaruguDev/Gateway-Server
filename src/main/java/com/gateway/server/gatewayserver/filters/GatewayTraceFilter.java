package com.gateway.server.gatewayserver.filters;
...
public class GatewayTraceFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if(isCorrelationIdPresent(requestHeaders)){
            logger.debug("x-correlation-id found in tracing filter: {}. ",
                    filterUtility.getCorrelationId(requestHeaders));
        }else{
            String correlationId = generateCorrelationId();
            exchange = filterUtility.setCorrelationId(exchange, correlationId);
            logger.debug("x-correlation-id generated in tracing filter: {}. ", correlationId);
        }
